...
![Figure 1. Map of Alabama and 9 Wildlife Management Areas](FinalProject\Map_Alabama.jpg)

![Figure 2. Landcover Classifications on 9 Wildlife Management Areas](FinalProject\Landcover.png)
...
![Figure 3. Landcover classifications at Perdido River WMA with count of target species in each landcover type ](FinalProject\targetspecies_PR.jpeg)

# Occupancy Models of Target Species 

##NOBO 

```{r}
# NOBO Occupancy on WMA Landcover Classifications 

# Load and prepare vegetation data
...
points = points[points$wma == "PR",]

#Remove individuals where the bird FLUSHED at "Time of First Detection" 
individuals$timeOfFirstDet[is.na(individuals$timeOfFirstDet)] <- "FLUSHED"


#Filter distance 
filtered_data <- merged_data[!is.na(merged_data$dataSheetNum), ]
...
filtered_data <- filtered_data %>%
  group_by(pointId2) %>%
  mutate(prawPresence = ifelse(any(species == "PRAW"), 1, 0)) %>%
  ungroup()
filtered_data <- filtered_data %>%
  relocate(prawPresence, .before = 35)

valid_species <- filtered_data %>%
  filter(!str_starts(species, "XX")) %>%
  pull(species) %>%
  unique()
...
presence_cols <- grep("Presence$", colnames(filtered_data), value = TRUE)
other_cols <- setdiff(colnames(filtered_data), presence_cols)
filtered_data <- filtered_data %>%
...
NOBO_model <- occu(~ observer + scaledTimeOfDay ~ landcover + BACSPresence + PRAWPresence, data = unmarked_occu_data)
...
# Interpretation of Occupancy Model

**Occupancy (logit-scale)**

- Intercept: The baseline logit of occupancy for the reference landcover category.
- landcover: The effect of different landcover types on occupancy probability.
- Estimate: The estimated effect size.
- SE: Standard error of the estimate.
- z: The z-value (estimate divided by SE).
- P(>|z|): The p-value indicating the significance of the effect.

**Detection (logit-scale)**

- Intercept: The baseline logit of detection probability for the reference observer.
- observer: The effect of different observers on detection probability.
- scaledTimeOfDay: The effect of the time of day on detection probability.

**Key Points**

- Significant Effects: Look for p-values less than 0.05 to identify significant effects.
- landcoverUAP: Significant negative effect on occupancy (p = 0.0336).
- observerJFH, observerJS, observerLAD, observerLKP, observerMSS, observerWMK: Significant positive effects on detection.
- scaledTimeOfDay: Significant negative effect on detection (p = 0.000968).

**AIC (Akaike Information Criterion)**

-AIC: A measure of model fit. Lower values indicate a better fit.
Number of Sites
-85 sites were included in the analysis.
Sites removed due to NA: Sites 8, 10, 45, and 66 were excluded due to missing data.

**Landcover Table**

EAP: 31 points
HardwoodWetland: 10 points
MPH: 9 points
OP: 15 points
UAP: 8 points
YP: 20 points

# Results of Occupancy Model

For all 9 WMAs: The model shows that across all WMAs, the following have a positive effect on NOBO presence, but none of them are significatn:
- Young Pine(YP)
- Open Areas (OP)
- Even-Aged Pine (EAP)
...
occupancy_results <- data.frame(
  ...
  estimate = c(2.03, -2.71, 11.91, 15.11, -3.55, -1.16),
  se = c(1.26, 1.52, 584.13, 4332.03, 1.67, 1.41)
)

# Enhanced Plotting with effect size on x-axis, no y-axis, annotations, and a legend
ggplot(occupancy_results, aes(x = estimate, y = landcover, color = landcover)) +
  geom_point(size = 4) +
  ...
  xlim(-15, 15)   # Adjust the limits as needed
```
