...
surveys <- read_excel("tidyData\\birdsICP2.xlsx",sheet = "surveyLevel")  # Data containing survey-level information
individuals <- read_excel("tidyData\\birdsICP2.xlsx",sheet = "individualLevel")
points <- read_excel("tidyData\\vegICP2.xlsx",sheet = "pointLevel") #Data containing point-level information
detections <- read_excel("tidyData\\birdsICP2.xlsx",sheet = "detectionLevel") #Data containing detection-level information
...
```{r}
##################MAY WANT TO MOVE THIS UP TO TIDY SECTION #####################
...
#Species Specific Exporation

##NOBO 

### NOBO Grouped by WMA, Landcover, and WMA & Landcover

```{r}

################ WMA ##################
#Count occurences of species "NOBO" grouped by WMA 
nobo_counts_wma <- merged_data %>%
  filter(species == "NOBO") %>%  # Retain only rows where species is "NOBO"
  group_by(wma) %>%              # Group the data by WMA
  summarise(count = n())         # Count the number of occurrences within each WMA group

# Print the counts of "NOBO" grouped by WMA
print(nobo_counts_wma)


################ Landcover ##################
# Count occurrences of species "NOBO" grouped by landcover
nobo_counts_landcover <- merged_data %>%
  filter(species == "NOBO") %>%  # Retain only rows where species is "NOBO"
  group_by(landcover) %>%        # Group the data by landcover
  summarise(count = n())         # Count the number of occurrences within each landcover group

# Print the counts of "NOBO" grouped by landcover
print(nobo_counts_landcover)

################ WMA and Landcover ##################
# Count occurrences of species "NOBO" grouped by both WMA and landcover
nobo_counts_wma_landcover <- merged_data %>%
  filter(species == "NOBO") %>%  # Retain only rows where species is "NOBO"
  group_by(wma, landcover) %>%   # Group the data by WMA and landcover
  summarise(count = n())         # Count the number of occurrences within each WMA-landcover group

# Print the counts of "NOBO" grouped by WMA and landcover
print(nobo_counts_wma_landcover)
```

## BACS

### BACS Grouped by WMA, Landcover, and WMA & Landcover

```{r}

################ WMA ##################
#Count occurences of species "BACS" grouped by WMA 
BACS_counts_wma <- merged_data %>%
  filter(species == "BACS") %>%  # Retain only rows where species is "BACS"
  group_by(wma) %>%              # Group the data by WMA
  summarise(count = n())         # Count the number of occurrences within each WMA group

# Print the counts of "BACS" grouped by WMA
print(BACS_counts_wma)

################ Landcover ##################
# Count occurrences of species "BACS" grouped by landcover
BACS_counts_landcover <- merged_data %>%
  filter(species == "BACS") %>%  # Retain only rows where species is "BACS"
  group_by(landcover) %>%        # Group the data by landcover
  summarise(count = n())         # Count the number of occurrences within each landcover group

# Print the counts of "BACS" grouped by landcover
print(BACS_counts_landcover)

################ WMA and Landcover ##################
# Count occurrences of species "BACS" grouped by both WMA and landcover
BACS_counts_wma_landcover <- merged_data %>%
  filter(species == "BACS") %>%  # Retain only rows where species is "BACS"
  group_by(wma, landcover) %>%   # Group the data by WMA and landcover
  summarise(count = n())         # Count the number of occurrences within each WMA-landcover group

# Print the counts of "BACS" grouped by WMA and landcover
print(BACS_counts_wma_landcover)
```

## AMKE

### AMKE Grouped by WMA, Landcover, and WMA & Landcover

```{r}

################ WMA ##################
#Count occurences of species "AMKE" grouped by WMA 
AMKE_counts_wma <- merged_data %>%
  filter(species == "AMKE") %>%  # Retain only rows where species is "AMKE"
  group_by(wma) %>%              # Group the data by WMA
  summarise(count = n())         # Count the number of occurrences within each WMA group

# Print the counts of "AMKE" grouped by WMA
print(AMKE_counts_wma)

################ Landcover ##################
# Count occurrences of species "AMKE" grouped by landcover
AMKE_counts_landcover <- merged_data %>%
  filter(species == "AMKE") %>%  # Retain only rows where species is "AMKE"
  group_by(landcover) %>%        # Group the data by landcover
  summarise(count = n())         # Count the number of occurrences within each landcover group

# Print the counts of "AMKE" grouped by landcover
print(AMKE_counts_landcover)

################ WMA and Landcover ##################
# Count occurrences of species "AMKE" grouped by both WMA and landcover
AMKE_counts_wma_landcover <- merged_data %>%
  filter(species == "AMKE") %>%  # Retain only rows where species is "AMKE"
  group_by(wma, landcover) %>%   # Group the data by WMA and landcover
  summarise(count = n())         # Count the number of occurrences within each WMA-landcover group

# Print the counts of "AMKE" grouped by WMA and landcover
print(AMKE_counts_wma_landcover)
```
